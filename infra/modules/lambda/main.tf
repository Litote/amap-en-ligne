...
locals {
  ...
  # INSTANCE_GUIDE_URL is optional — omitted from env when not configured.
  guide_env = var.instance_guide_url != "" ? { INSTANCE_GUIDE_URL = var.instance_guide_url } : {}
}
...
resource "aws_lambda_function" "data" {
  ...
  environment {
    ...
    }, local.push_env, local.terms_env, local.guide_env)
  }
