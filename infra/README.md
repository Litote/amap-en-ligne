...
deploy. The deployer policy needs `cloudfront:CreateInvalidation` (added to
`bootstrap/deployer.tf`) — **re-apply `bootstrap/` before the first run** of this
workflow, otherwise the invalidation step fails with `AccessDenied`.
...
#### `INSTANCE_GUIDE_URL` (optional)

Both deployments read the optional `INSTANCE_GUIDE_URL` env var. When set, its
value is published in `GET /.well-known/amap-en-ligne.json` as `guide_url` and the
app's help screen links to it as the end-user guide.

- **Lambda**: governed by the `instance_guide_url` Terraform variable, which
  defaults to the project's published guide
  (`https://litote.github.io/amap-en-ligne/`). Set it to `""` in
  `terraform.tfvars` to omit the field, or point it at an instance-specific guide.
- **JVM**: set `INSTANCE_GUIDE_URL` in `back/deploy/jvm/.env`.
...
| **CloudFront** | Distribution, origin access control, response-headers policy, cache invalidation | `*` (no resource-level support) |
