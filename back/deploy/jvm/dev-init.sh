...
#
# Plus an empty organization "amap-import" with a single validated admin, ready
# to receive an OrganizationExport via POST /v1/admin/organizations/{id}/import
# (the import requires an empty target — members are not counted as content):
#
#   import-admin@DEV_DOMAIN                 roles: [ADMIN]  (org: amap-import)
set -euo pipefail
...
-- Empty org reserved for import testing (no producers / contracts / templates /
-- deliveries / invitations / join requests), kept importable on purpose.
INSERT INTO organization (organization_id, name, contact_email)
VALUES ('amap-import', 'AMAP Import (empty)', 'contact@amap-import.${DEV_DOMAIN}')
ON CONFLICT (organization_id) DO NOTHING;
SQL
...
# Admin of the empty "amap-import" org, ready to import an OrganizationExport.
seed_user "import-admin@${DEV_DOMAIN}" \
  '{"roles":["ADMIN"]}'
...
-- Admin member of the empty amap-import org. ImportService resolves the caller's
...
print_user "IMPORT ADMIN"      "import-admin@${DEV_DOMAIN}"
printf '\n(import-admin is ADMIN of the empty "amap-import" org, ready for import)\n'
printf '\n'
