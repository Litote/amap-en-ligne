...
abstract class JvmSyncTestSupport {
    ...
    protected fun postOwnerInvitationMutation(
        ...
        // Each distinct user action gets its own clientOpId (reusing one would
        // legitimately replay the stored idempotent outcome instead of re-applying).
        clientOpId: String = "owner-invite",
    ): SyncResponse =
        postSyncAs(
            token = ownerToken,
            request =
