...
class JvmDeploymentIntegrationTest : JvmSyncTestSupport() {
    ...
    fun `GIVEN OWNER caller and duplicate email WHEN owner invitation mutation repeated THEN second is rejected`() =
        runTest {
            ...
            val first = postOwnerInvitationMutation(ownerToken, email = email, clientOpId = "owner-invite-1")
            assertEquals(
                "APPLIED",
                first.mutations
                    .single()
            ...
            val second = postOwnerInvitationMutation(ownerToken, email = email, clientOpId = "owner-invite-2")
            assertEquals(
                "REJECTED",
                second.mutations
                    .single()
