 ...
 *
 * [clientOpId] is the client-chosen idempotency key: the server persists an
 * `AppliedClientOp` record per APPLIED mutation and replays the stored
 * [MutationOutcome] (same `serverEntityId`) when the same clientOpId is
 * resent by the same caller — a retried `tmp_*` creation never allocates a
 * second entity. REJECTED outcomes are not persisted (re-evaluating them is
 * deterministic and side-effect-free). It is also the join key the client
 * uses to correlate this mutation with its [MutationOutcome] in the response.
 *
