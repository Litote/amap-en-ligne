...
private val SELECT_PRODUCER_ORGANIZATIONS_SQL =
    """
    ...
    override suspend fun getByOrganizationId(organizationId: Id<Organization>): List<ProducerAccount> =
        client.dataSource.query { conn ->
            ...
            producerIds.map { producer ->
                producer.copy(
                    organizations = loadProducerOrganizations(conn, producer.producerAccountId.id),
                    products = loadProducerProducts(conn, producer.producerAccountId.id),
                )
            }
        }
        ...
        client.dataSource.query { conn ->
            ...
            producer?.let { base ->
                val organizations =
                    conn
                        .prepareStatement(
                            SELECT_PRODUCER_ORGANIZATIONS_SQL,
                        ).use { stmt ->
                            stmt.setString(1, producerAccountId.id)
                            stmt.executeQuery().use { rs ->
                                buildList {
...
private fun loadProducerOrganizations(
    conn: Connection,
    producerAccountId: String,
): List<ProducerOrganization> =
    conn
        .prepareStatement(SELECT_PRODUCER_ORGANIZATIONS_SQL)
        .use { stmt ->
...
private fun loadProducerProducts(
    conn: Connection,
    producerAccountId: String,
): List<ProducerProduct> =
    conn
        .prepareStatement(
            """
