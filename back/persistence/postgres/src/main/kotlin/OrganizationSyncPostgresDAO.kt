...
internal class OrganizationSyncPostgresDAO(
    ...
    override suspend fun getById(organizationId: Id<Organization>): Organization? =
        client.dataSource.query { conn ->
            val orgWithProducers = loadOrgWithProducers(conn, organizationId) ?: return@query null
            orgWithProducers.copy(products = loadOrgProducts(conn, organizationId))
        }

    private fun loadOrgWithProducers(
        conn: Connection,
