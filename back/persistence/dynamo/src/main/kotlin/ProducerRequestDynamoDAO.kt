...
private const val EXPRESSION_PK = "pk = :pk"
private const val VALUE_STATUS = ":status"
    ...
    override suspend fun existsByProducerName(
        ...
        val response =
            client.client.query(
                QueryRequest {
                    tableName = client.table
                    keyConditionExpression = EXPRESSION_PK
        ...
        val gsiResponse =
            client.client.query(
                QueryRequest {
                    ...
                    filterExpression = EXPRESSION_PK
                    expressionAttributeValues =
                        mapOf(
            ...
            client.client.query(
                QueryRequest {
                    ...
                    expressionAttributeValues =
                        mapOf(
                            ":pk" to AttributeValue.S(PRODREQ_PK),
                            VALUE_STATUS to AttributeValue.S(status.name),
                        )
                },
            )
        return response.items.orEmpty().map { it.toProducerRequest() }
