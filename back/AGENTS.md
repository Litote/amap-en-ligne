...
- [ ] `back/gradlew check` passes (compiles without warnings, all tests pass) — `check` is unit-only; the `AcceptanceScenariosTest` suite runs via the repo-root `./gradlew allAcceptanceTests`, not `check`
