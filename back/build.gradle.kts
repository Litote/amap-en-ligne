...
subprojects {
    afterEvaluate {
        ...
        if (path == ":deploy:lambda") {
            // main() only dispatches to the blocking native Lambda runtime loop —
            // it cannot run inside a unit test, so its lines stay uncovered.
            extensions.findByType(SonarExtension::class)?.properties {
                property("sonar.coverage.exclusions", "src/main/kotlin/Main.kt")
            }
        }
    }
}
