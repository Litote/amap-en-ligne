...
**Idempotency on retry.** The server persists an `AppliedClientOp` record
(`client_op_id`, caller sub, `serverEntityId`) for every **APPLIED**
mutation. When the same `client_op_id` is resent by the same caller —
e.g. the response was lost after the server committed — `DataService`
short-circuits and replays the stored outcome, so a retried `tmp_*`
creation never allocates a second real id. The replayed outcome still
feeds the in-batch `tmpIdMap`, so FK references in a retried batch resolve
to the originally allocated id. Details and accepted trade-offs:

- **Only APPLIED outcomes are stored.** The client drains its pending
  queue on *any* returned outcome (including REJECTED), so a rejected op
  is never retried once a response arrives; re-evaluating a rejection is
  deterministic and side-effect-free.
- **The record write is best-effort and non-atomic** with the entity write
  (a single mutation may span several DAO transactions — denormalised
  fan-outs). A server crash between the entity commit and the record write
  falls back to today's re-apply behavior — a drastically narrowed window.
  A failed record write is logged, never surfaced as a mutation failure.
- **Anti-spoofing**: the short-circuit only fires when the stored caller
  sub matches the authenticated caller; a foreign `client_op_id` is
  processed normally.
- **Retention**: DynamoDB items carry a `ttl` attribute (30 days, TTL
  enabled in Terraform); the Postgres `applied_client_op` table has no
  cleanup in v1 (accepted — rows are tiny and queues drain quickly).
