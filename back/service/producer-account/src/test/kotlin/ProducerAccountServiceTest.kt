...
internal class ProducerAccountServiceTest {
    ...
    private val upsertNormalizer = ProducerAccountUpsertNormalizer(producerAccountSyncDAO)
    private val service =
        ProducerAccountService(
            producerAccountSyncDAO = producerAccountSyncDAO,
            organizationSyncDAO = organizationSyncDAO,
            upsertNormalizer = upsertNormalizer,
            changeFactory = changeFactory,
    ...
    fun `GIVEN unknown producer WHEN suspend THEN NotFound`() =
        runTest {
            ...
            val result = lifecycleService.suspend(actorSub = "sub-owner", producerAccountId = "pa-1")
    ...
    @Test
    fun `GIVEN active ACCOUNT_BACKED producer with linked auth user WHEN suspend THEN banUser called`() =
        runTest {
            val producer = producer("pa-1", activeStatus = true)
            coEvery { producerAccountSyncDAO.findById(any()) } returns producer
            coEvery { producerAccountSyncDAO.updateActiveStatus(any(), any(), any()) } returns Unit
            coEvery { userProvisioningPort.listAuthSubsByProducerAccount("pa-1") } returns listOf("sub-producer-1")

            val result = lifecycleService.suspend(actorSub = "sub-owner", producerAccountId = "pa-1")

            assertIs<ProducerLifecycleOutcome.Success>(result)
            coVerify(exactly = 1) { userProvisioningPort.banUser("sub-producer-1") }
            coVerify(exactly = 0) { userProvisioningPort.unbanUser(any()) }
        }

    @Test
    fun `GIVEN suspended ACCOUNT_BACKED producer with linked auth user WHEN reactivate THEN unbanUser called`() =
        runTest {
            ...
            val result = lifecycleService.reactivate(actorSub = "sub-owner", producerAccountId = "pa-1")

            assertIs<ProducerLifecycleOutcome.Success>(result)
            coVerify(exactly = 1) { userProvisioningPort.unbanUser("sub-producer-1") }
            coVerify(exactly = 0) { userProvisioningPort.banUser(any()) }
        }

    @Test
    fun `GIVEN NO_ACCOUNT producer WHEN suspend THEN listAuthSubs returns empty and no ban call`() =
        runTest {
            val producer =
                producer("pa-no-account", activeStatus = true).copy(
                    managementMode = ProducerManagementMode.NO_ACCOUNT,
                    organizations = listOf(organizationLink("org-1")),
                )
            coEvery { producerAccountSyncDAO.findById(any()) } returns producer
            coEvery { producerAccountSyncDAO.updateActiveStatus(any(), any(), any()) } returns Unit
            coEvery { userProvisioningPort.listAuthSubsByProducerAccount("pa-no-account") } returns emptyList()

            val result = lifecycleService.suspend(actorSub = "sub-owner", producerAccountId = "pa-no-account")
    ...
    @Test
    fun `GIVEN actorSub equals producerAccountId WHEN suspend THEN SELF_ACTION_FORBIDDEN without DAO write`() =
        runTest {
            val producer = producer("pa-1", activeStatus = true)
            coEvery { producerAccountSyncDAO.findById(any()) } returns producer

            val result = lifecycleService.suspend(actorSub = "pa-1", producerAccountId = "pa-1")

            assertIs<ProducerLifecycleOutcome.Rejected>(result)
            assertEquals(MutationErrorCode.SELF_ACTION_FORBIDDEN, result.code)
            coVerify(exactly = 0) { producerAccountSyncDAO.updateActiveStatus(any(), any(), any()) }
        }

    @Test
    fun `GIVEN ban throws WHEN suspend THEN email side-effect still fires`() =
        runTest {
            val producer = producer("pa-1", activeStatus = true)
            coEvery { producerAccountSyncDAO.findById(any()) } returns producer
            coEvery { producerAccountSyncDAO.updateActiveStatus(any(), any(), any()) } returns Unit
            coEvery { userProvisioningPort.listAuthSubsByProducerAccount("pa-1") } returns listOf("sub-producer-1")
            coEvery { userProvisioningPort.banUser("sub-producer-1") } throws RuntimeException("auth provider unreachable")
    ...
    fun `GIVEN unknown producer WHEN delete THEN NotFound`() =
        runTest {
            ...
            val result = lifecycleService.delete(actorSub = "sub-owner", producerAccountId = "pa-1")
    ...
    @Test
    fun `GIVEN userPreferences provided in update WHEN updateProfile THEN userPreferences persisted`() =
        runTest {
            val producer = producer("pa-1", activeStatus = true)
            val updatedSlot = slot<ProducerAccount>()
            coEvery { producerAccountSyncDAO.findById(any()) } returns producer
            ...
            val newPreferences =
                UserPreferences(
                    ...
                    lastUpdatedInstant = Instant.fromEpochMilliseconds(5_000_000L),
                )
            ...
            assertIs<ProducerLifecycleOutcome.Success>(result)
            assertEquals(newPreferences, updatedSlot.captured.userPreferences)
        }

    @Test
    fun `GIVEN null userPreferences in update WHEN updateProfile THEN existing userPreferences preserved`() =
        runTest {
            val existingPreferences =
                UserPreferences(
                    emailNotificationsEnabled = false,
                    pushNotificationsEnabled = true,
                    lastUpdatedInstant = Instant.fromEpochMilliseconds(3_000_000L),
                )
            val producer = producer("pa-1").copy(userPreferences = existingPreferences)
            val updatedSlot = slot<ProducerAccount>()
            coEvery { producerAccountSyncDAO.findById(any()) } returns producer
            coEvery { producerAccountSyncDAO.updateProfile(capture(updatedSlot), any()) } returns Unit
            ...
            lifecycleService.updateProfile(
                producerAccountId = "pa-1",
                update = ProducerAccountProfileUpdate("Farm", null, null, null, userPreferences = null),
            )

            assertEquals(existingPreferences, updatedSlot.captured.userPreferences)
        }
