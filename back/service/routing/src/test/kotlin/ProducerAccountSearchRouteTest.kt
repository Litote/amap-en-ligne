...
internal class ProducerAccountSearchRouteTest {
    ...
    private fun buildStubProducerAccount(
        id: String = "pa-1",
        activeStatus: Boolean = true,
        managementMode: ProducerManagementMode = ProducerManagementMode.ACCOUNT_BACKED,
    ...
    // ── OWNER search (instance-wide) ──────────────────────────────────────────

    private fun buildKoinForOwner(producerAccountSyncDAO: ProducerAccountSyncDAO) =
        startKoin {
            modules(
                module {
                    single<DataService> { mockk(relaxed = true) }
                    single<AuthenticationService> { ownerAuthService }
                    single { HttpService() }
                    single { stubInstanceConfig }
                    single { mockk<PublicService>(relaxed = true) }
                    single { mockk<AdminService>(relaxed = true) }
    ...
    @Test
    fun `GIVEN OWNER token and query matching a producer from any org WHEN GET search THEN returns 200 with that producer`() =
        runTest {
            val producerAccountSyncDAO = mockk<ProducerAccountSyncDAO>()
            coEvery { producerAccountSyncDAO.listAll() } returns listOf(buildStubProducerAccount("pa-owner-1"))

            val koin = buildKoinForOwner(producerAccountSyncDAO)
            ...
            testApplication {
                ...
                assertEquals(HttpStatusCode.OK, response.status)
                assertTrue(response.bodyAsText().contains("pa-owner-1"))
            }
        }

    @Test
    fun `GIVEN OWNER token and inactive producer matching query WHEN GET search THEN producer excluded`() =
        runTest {
            val producerAccountSyncDAO = mockk<ProducerAccountSyncDAO>()
            coEvery { producerAccountSyncDAO.listAll() } returns
                listOf(buildStubProducerAccount("pa-inactive", activeStatus = false))
    ...
    @Test
    fun `GIVEN OWNER token and NO_ACCOUNT producer matching query WHEN GET search THEN producer excluded`() =
        runTest {
            val producerAccountSyncDAO = mockk<ProducerAccountSyncDAO>()
            coEvery { producerAccountSyncDAO.listAll() } returns
                listOf(
                    buildStubProducerAccount(
                        "pa-no-account",
    ...
    @Test
    fun `GIVEN OWNER token and query matching producer contactEmail WHEN GET search THEN producer included`() =
        runTest {
            val producerAccountSyncDAO = mockk<ProducerAccountSyncDAO>()
            // Producer whose name does not match but contactEmail does
            val pa =
                buildStubProducerAccount("pa-email-match").copy(
                    name = "Domaine XYZ",
                    contactEmail = "contact@ferme-bio.fr",
                )
            coEvery { producerAccountSyncDAO.listAll() } returns listOf(pa)
            ...
            testApplication {
                ...
                val response =
                    client.get("/v1/admin/producer-accounts/search?q=ferme-bio") {
                        header(HttpHeaders.Authorization, "Bearer owner-token")
                    }

                assertEquals(HttpStatusCode.OK, response.status)
                assertTrue(response.bodyAsText().contains("pa-email-match"))
            }
        }
}
