...
class OwnerService(
    ...
    suspend fun updateProfile(
        ...
        val updated =
            owner.copy(
                ...
                email = update.email,
                phone = update.phone,
                userPreferences = update.userPreferences ?: owner.userPreferences,
    ...
    private suspend fun transition(
        ...
        val owner = ownerDAO.findById(ownerId.toOwnerId()) ?: return OwnerLifecycleOutcome.NotFound
        val suspending = target == AccountStatus.SUSPENDED
        if (owner.ownerId.id == actorSub) {
            return rejectSelfAction(if (suspending) "suspend" else "reactivate")
        }
        if (suspending) {
            val lastOwnerError = roleService.validateLastOwner(ownerId)
            if (lastOwnerError != null) {
        ...
        // Idempotent: skip the row + Change write when already in the target state,
        // but still fire side-effects below so the auth provider stays in sync.
        ...
        fireLifecycleSideEffects(owner, actorSub, suspending)
        return OwnerLifecycleOutcome.Success
    }

    private suspend fun fireLifecycleSideEffects(
        owner: Owner,
        actorSub: String,
        suspending: Boolean,
    ) {
