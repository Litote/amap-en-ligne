...
internal class OwnerTypeServiceTest {
    ...
    @Test
    fun `GIVEN OWNER caller self-upsert with changed userPreferences WHEN applyUpsert THEN userPreferences persisted`() =
        runTest {
            val selfOwnerAuth =
                AuthenticatedInfo(
                    memberId = "owner-1",
            ...
            val incoming = buildOwner(ownerId = "owner-1").copy(userPreferences = newPreferences)
            coEvery { ownerDAO.findById("owner-1".toId()) } returns existingOwner
            coEvery { ownerDAO.findBySub("owner-1") } returns existingOwner
            val updatedSlot = io.mockk.slot<Owner>()
            coEvery { ownerDAO.put(capture(updatedSlot), any()) } returns Unit
