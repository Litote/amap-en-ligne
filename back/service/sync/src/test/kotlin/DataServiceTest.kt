...
import produceraccount.ProducerAccountChangeFactory
...
/** Map-backed [AppliedClientOpDAO] so tests can exercise the replay path. */
private class InMemoryAppliedClientOpDAO : AppliedClientOpDAO {
    val records = mutableMapOf<String, AppliedClientOp>()

    override suspend fun record(entry: AppliedClientOp) {
        records.putIfAbsent(entry.clientOpId, entry)
    }

    override suspend fun find(clientOpId: String): AppliedClientOp? = records[clientOpId]
}

internal class DataServiceTest {
    private val appliedClientOpDAO = InMemoryAppliedClientOpDAO()
    ...
    // Builds a DataService around a ProductTypeService for the replay tests.
    private fun productTypeSyncService(productTypeDAO: ProductTypeSyncDAO): DataService =
        DataService(
            services =
                listOf(
                    ProductTypeService(productTypeDAO),
    ...
    @Test
    fun `GIVEN a retried tmp creation WHEN sync THEN replays the stored outcome without re-applying`() =
        runTest {
            val productTypeDAO = mockk<ProductTypeSyncDAO>(relaxed = true)
            val service = productTypeSyncService(productTypeDAO)
            val mutation =
                ClientMutation(
                    clientOpId = "op-retry",
                    op = Upsert(ProductTypePayload(productType.copy(productTypeId = "tmp_veg".toId()))),
                )

            val first = service.sync(producerAuth, emptyMap(), listOf(mutation)).mutations.single()
            val second = service.sync(producerAuth, emptyMap(), listOf(mutation)).mutations.single()

            assertEquals(MutationStatus.APPLIED, first.status)
            assertEquals(MutationStatus.APPLIED, second.status)
            assertEquals(first.serverEntityId, second.serverEntityId)
            coVerify(exactly = 1) { productTypeDAO.put(any(), any()) }
        }

    @Test
    fun `GIVEN a retried batch WHEN sync THEN the replayed tmp id still rewrites later FK references`() =
        runTest {
            // First sync applies only the tmp_* producer creation; the retried batch
            // resends it together with an Organization referencing the tmp id. The
            // replayed outcome must feed the tmpIdMap so the FK resolves to the id
            // allocated on the FIRST sync, not a fresh one.
            val organizationId = "org-replay"
            val tmpId = "tmp_replayed-producer"

            val adminAuth =
                AuthenticatedInfo(
                    memberId = "admin-replay",
                    firstName = "Admin",
                    lastName = "User",
            ...
            val service =
                DataService(
                    services =
                        listOf(
                            ProducerAccountService(
                                ...
                                upsertNormalizer =
                                    ProducerAccountUpsertNormalizer(
                                        producerAccountSyncDAO = producerAccountSyncDAO,
                                    ),
                                changeFactory = ProducerAccountChangeFactory(),
                                lifecycleService =
                                    ProducerAccountLifecycleService(
                                        producerAccountSyncDAO = producerAccountSyncDAO,
                    ...
                    appliedClientOpDAO = appliedClientOpDAO,
                    memberSyncDAO = mockk(relaxed = true),
                    authorizedScopeResolver = testScopeResolverForAdmin("admin-replay", organizationId),
                )

            val createMutation =
                ClientMutation(
                    clientOpId = "op-producer-replay",
                    op = Upsert(ProducerAccountPayload(producerAccount)),
                )
            val orgMutation =
                ClientMutation(
                    clientOpId = "op-org-replay",
                    op = Upsert(OrganizationPayload(organization)),
                )

            val cursors = mapOf("organization:$organizationId" to "c-init")
            val firstId =
                service
                    .sync(adminAuth, cursors, listOf(createMutation))
                    .mutations
                    .single()
                    .serverEntityId
            assertTrue(firstId != null && !firstId.startsWith(ClientMutation.TMP_ID_PREFIX))

            val retried = service.sync(adminAuth, cursors, listOf(createMutation, orgMutation)).mutations

            assertEquals(firstId, retried.first { it.clientOpId == "op-producer-replay" }.serverEntityId)
            assertEquals(MutationStatus.APPLIED, retried.first { it.clientOpId == "op-org-replay" }.status)
            // The producer was applied only once...
            coVerify(exactly = 1) { producerAccountSyncDAO.put(any(), any(), any()) }
            // ...and the Organization stored on the retry carries the ORIGINAL real id.
            assertEquals(
                firstId,
    ...
    @Test
    fun `GIVEN a stored op from another caller WHEN sync THEN the mutation is applied normally`() =
        runTest {
            val productTypeDAO = mockk<ProductTypeSyncDAO>(relaxed = true)
            val service = productTypeSyncService(productTypeDAO)
            appliedClientOpDAO.records["op-foreign"] =
                AppliedClientOp(
                    clientOpId = "op-foreign",
                    callerSub = "someone-else",
                    entityType = EntityType.ProductType,
                    serverEntityId = "stolen-id",
                    appliedAt = Instant.fromEpochMilliseconds(1),
                )
            ...
            assertEquals(productType.productTypeId.id, outcome.serverEntityId)
            coVerify(exactly = 1) { productTypeDAO.put(any(), any()) }
        }

    @Test
    fun `GIVEN a rejected mutation WHEN sync THEN no idempotency record is written`() =
        runTest {
            val productTypeDAO = mockk<ProductTypeSyncDAO>(relaxed = true)
            val service = productTypeSyncService(productTypeDAO)
            val rejected =
                ClientMutation(
                    ...
                    op =
                        Upsert(
                            ProductTypePayload(
                                productType.copy(producerAccountId = "other-producer".toId()),
                            ),
                        ),
                )

            val outcome = service.sync(producerAuth, emptyMap(), listOf(rejected)).mutations.single()

            assertEquals(MutationStatus.REJECTED, outcome.status)
            assertNull(appliedClientOpDAO.records["op-rejected"])
        }

    @Test
    fun `GIVEN an entity service throwing WHEN applying a mutation THEN outcome is REJECTED with INTERNAL_ERROR`() =
        runTest {
            val productTypeDAO = mockk<ProductTypeSyncDAO>(relaxed = true)
            coEvery { productTypeDAO.put(any(), any()) } throws IllegalStateException("boom")
            val service = productTypeSyncService(productTypeDAO)
            ...
            assertEquals(MutationStatus.REJECTED, outcome.status)
            assertEquals(MutationErrorCode.INTERNAL_ERROR, outcome.error?.code)
            assertNull(appliedClientOpDAO.records["op-internal-error"])
        }
