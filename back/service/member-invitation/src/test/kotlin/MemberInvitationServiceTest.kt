...
internal class MemberInvitationServiceTest {
    ...
    private val ownerSyncDAO = mockk<OwnerSyncDAO>(relaxed = true)
    private val service =
    ...
    @Test
    fun `GIVEN email already belongs to an owner WHEN applyUpsert THEN rejects with unique violation`() =
        runTest {
            coEvery { memberSyncDAO.getByOrganizationId(any()) } returns emptyList()
            coEvery { ownerSyncDAO.existsByEmail("alice@example.com") } returns true
