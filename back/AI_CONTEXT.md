...
**Retry idempotency**: every APPLIED mutation persists an `AppliedClientOp` record (`AppliedClientOpDAO`, keyed by `client_op_id`, caller-sub-pinned; Dynamo item `pk=APPLIED_OP` with 30-day `ttl`, Postgres `applied_client_op` table in `V1__init.sql` — migrations squashed, no cleanup in v1). A resent `client_op_id` from the same caller short-circuits in `DataService` and replays the stored outcome (same `serverEntityId`, so a retried `tmp_*` creation never double-creates); the replayed outcome still populates the in-batch `tmpIdMap`. Only APPLIED outcomes are stored (the client drains pending rows on any outcome, incl. REJECTED — except `INTERNAL_ERROR`, which the client keeps queued and retries); the record write is best-effort *after* the entity commit (non-atomic — a crash in between narrows to the old re-apply behavior). `DataService.applyMutation`'s catch-all maps unexpected exceptions to `REJECTED`/`INTERNAL_ERROR` (never `INVALID_PAYLOAD`, which would make the client drop the mutation). Pinned by `DataServiceTest` replay cases + `acceptance/scenarios/sync-retry-idempotent-create-product-type.json`.
...
| `Organization` (`organization:{id}`; OWNER snapshot = all orgs) | `OrganizationService` — rejects duplicate `deliveries[].scheduledDate.date` (`UNIQUE_VIOLATION`); `MISSING_COORDINATOR` guard on CONFIRMED deliveries; `INVALID_PAYLOAD` guard when a newly added `DeliveryContract.coordinators` entry is not in the linked `Contract.coordinators` pool (`checkCoordinatorsInContractPool`, privileged path, new entries only); `CONTRACT_ENDED` guard on newly linked ended contracts (privileged path; existing links stay editable); delegates VOLUNTEER-only callers to `VolunteerMutationValidator` and privileged slot writes to `SlotLifecycleNormalizer`. |
| `ProducerAccount` (org scopes + `instance-owner`) | `ProducerAccountService` — ADMIN may create/update `NO_ACCOUNT` producers (mode immutable; single org auto-normalised; derives `Organization.products` from `ProducerAccount.products`); OWNER lifecycle: suspend/reactivate (`active_status` flip, bans/unbans every linked auth sub via `UserProvisioningPort.banUser`/`unbanUser`, no-op for `NO_ACCOUNT`; rejects an OWNER suspending/reactivating their own producer account with `SELF_ACTION_FORBIDDEN`) and delete (auth users deleted, row kept inactive, `AccountDeletionLog` per sub). |
...
| `MemberInvitation` (org) / `OwnerInvitation` (`instance-owner`) | `MemberInvitationService` / `OwnerInvitationService` — `tmp_*` upsert creates invitation + token + email (rejects `UNIQUE_VIOLATION` when the email already has a same-org `Member` row, a non-expired pending invitation, **or belongs to an instance `Owner`** — owner-exclusivity guard via `OwnerSyncDAO.existsByEmail`); upsert with strictly newer `resend_requested_at` invalidates old tokens and resends (idempotency cursor); `Delete` cancels (`status=CANCELLED`, tokens invalidated). `MemberInvitation` carries optional `custom_email_subject` / `custom_email_body` (persisted, forwarded to `EmailTemplates.memberInvitation`); resend persists the incoming values (the resend contract only pins identity fields, so the custom copy may change). |
...
`MutationErrorCode`: `NOT_FOUND`, `FORBIDDEN`, `INVALID_PAYLOAD`, `INTERNAL_ERROR` (transient server failure — the only retryable code), `UNIQUE_VIOLATION`, `CONFLICT`, `OWNER_EXCLUSIVE`, `PRODUCER_EXCLUSIVE`, `MIXED_ROLES`, `LAST_ADMIN`, `LAST_OWNER`, `LAST_PRODUCER`, `SELF_ACTION_FORBIDDEN`, `MISSING_COORDINATOR`, `CONTRACT_ENDED`, `INVALID_SUBSCRIPTION`, `INVALID_SHARED_BASKET`.
...
- `/.well-known/*` — instance discovery (`DiscoveryRoute`, env: `INSTANCE_NAME`, `INSTANCE_API_URL`, optional `INSTANCE_VISIBLE`; carries `server_version` = `BuildInfo.VERSION` and nullable `server_commit` = `BuildInfo.COMMIT`, constants generated at build time by the `generateBuildInfo` task in `lib:instance-config` from `VERSION_NAME` resp. the `GIT_COMMIT_SHA`/`GITHUB_SHA` env — release-bumped, distinct from `protocol_version`; commit omitted on local builds) + app links (`DeepLinkRoute`, 404 unless `IOS_TEAM_ID`/`IOS_BUNDLE_ID` resp. `ANDROID_PACKAGE_NAME`/`ANDROID_CERT_FINGERPRINT` set).
...
- `GET /v1/admin/producer-accounts/search?q=` — authenticated REST exception (ADMIN/OWNER; full-table scan, acceptable at current volume). Returns only active `ACCOUNT_BACKED` producers matching `q` case-insensitively on name/contact-email. ADMIN path (`ProducerAccountSyncDAO.search`) excludes producers already linked/chosen by the caller org. **OWNER path** is instance-wide — `producerAccountSyncDAO.listAll()` filtered in-route by the same predicate, no org exclusion (the OWNER already has full visibility via `instance-owner`).
- `GET /v1/admin/organizations/{id}/export` + `POST /v1/admin/organizations/{id}/import` (`OrganizationBackupRoute` → `ExportService` / `ImportService` in `service:sync`; wired via `getOrNull` so manual-module route tests stay green). Native-JSON backup / migration of one org. **Export** reuses `DataService.snapshotScope(auth, organization:{id})` + `ProductTypeSyncDAO` for the linked producers' catalogs, returning a versioned `OrganizationExport` (`persistence:wire`). **Import** is a *trusted restore*: entities are written directly through the sync DAOs (bypassing the interactive `EntityTypeService` guards — role exclusivity, ACCOUNT_BACKED creation, coordinator/contract/capacity checks) with a fresh `Change` per entity on `organization:{id}` (ProductTypes on `producer-account:{id}`). Ids are **preserved**; only the source `organizationId` is rewritten to the target. Target must be **empty** (else `ImportOutcome.Conflict` → `409`); unknown version → `400`; bad role/org → `403`. Members import carries PII but no auth `sub`. **Import auto-generates a `PENDING_ACTIVATION` `MemberInvitation`** (org-scope `Change`, no token, no email) for every imported member with an email that has no imported invitation (deduped by lowercased email) — so the admin can later trigger "Demander la connexion" (`MemberInvitationService.resendInvitation`, which mints the token + sends the email); without this the member would be unreachable (`createInvitation` rejects emails that already have a `Member` row). `ImportResult.generatedInvitations` reports the count. **Owner-exclusivity skip**: a member whose email already belongs to an instance `Owner` (`OwnerSyncDAO.listAll`, case-insensitive) is dropped from the import along with its invitation (account + invitation both skipped, not counted in `members`), and the collision is surfaced in `ImportResult.warnings` for the UI. Auth in both services: OWNER any org, else ADMIN of that org.
