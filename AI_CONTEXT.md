...
`.github/workflows/ci.yml` runs three coverage-producing jobs in parallel — `back` (`./gradlew check`, unit only), `front` (`frontTest --coverage` lcov), `acceptance` (`./gradlew allAcceptanceTests`: back acceptance scenarios + cross-component E2E) — and a `sonar` job that `needs:` all three and runs a **scan-only** SonarCloud analysis. SonarCloud coverage is the **union** of the three JaCoCo XML / lcov reports (unit ∪ acceptance ∪ e2e), merged via `-Dsonar.coverage.jacoco.xmlReportPaths` (multiple paths) + `-Dsonar.dart.lcov.reportPaths`. Build mechanics: `jacocoExecReport` (`back/build.gradle.kts`) aggregates whatever `.exec` files a job produced — including `acceptance/e2e/build/jacoco/*.exec` (the e2e back server boots in-JVM) — with no test dependency; the scan-only `sonar` run is gated by `-PsonarExternalCoverage`. **Back `acceptanceTest` is NOT part of `check`** — it runs only via `allAcceptanceTests`; locally, `./gradlew allSonar` still folds in back acceptance (and any present e2e exec).
