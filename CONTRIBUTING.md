...
Since PRs are squash-merged, the PR title becomes the commit message on `main` and **drives the next release version** (see below): `fix:` → patch, `feat:` → minor, `feat!:` / `BREAKING CHANGE:` → major. **While the project is pre-1.0**, bumps are damped (`bump-minor-pre-major` + `bump-patch-for-minor-pre-major`): `fix:` and `feat:` both bump patch, breaking changes bump minor — versions stay `0.0.x`/`0.x.y` until 1.0.0 is promoted deliberately (merge a commit with a `Release-As: 1.0.0` footer).

## Releases & versioning

Versioning is automated by [release-please](https://github.com/googleapis/release-please) (`release-please.yml`):

- Every push to `main` updates a running **release PR** that accumulates the pending Conventional Commits into a `CHANGELOG.md` entry and the next semver bump.
- **Merging the release PR** is the release: it bumps the version everywhere and tags `vX.Y.Z`.

One single version covers the whole monorepo. The bump is propagated through `release-please-config.json` (`extra-files` + `x-release-please` markers) and the manifest `.release-please-manifest.json`:

| File | Mechanism |
|------|-----------|
| `version.txt` + `CHANGELOG.md` | `release-type: simple` (created/updated at repo root) |
...
At runtime, the back exposes what it was built from in the discovery document (`GET /.well-known/amap-en-ligne.json`): `server_version` (the release, baked from `VERSION_NAME` by the `generateBuildInfo` task in `back/lib/instance-config`) and `server_commit` (short SHA from `GIT_COMMIT_SHA`/`GITHUB_SHA` at build time; omitted on local builds). Since deploys happen on every push to `main` — not only on releases — `server_commit` is what identifies the deployed build exactly.
...
| `RELEASE_PLEASE_APP_ID` | `release-please` | GitHub App id used to mint the release PR token (the built-in `GITHUB_TOKEN` cannot trigger CI on the PRs it opens). Provided as a Litote **organization** secret. |
| `RELEASE_PLEASE_APP_PRIVATE_KEY` | `release-please` | Private key of that GitHub App (needs `contents: write` + `pull-requests: write` on this repository). Provided as a Litote **organization** secret. |
