...
class ProducerAccountRepository {
  ...
  /// so the back applies the change on the `producer-account:{id}` scope.
  Future<void> updateUserPreferences(
    ...
    final existing = await _db.watchProducerAccountById(producerAccountId).first;
    if (existing == null) return;
    final clientOpId = _idGen.next();
    await _db.enqueuePendingMutation(
      ClientMutation(
        clientOpId: clientOpId,
        op: Upsert(payload: ProducerAccountPayload(producerAccount: existing)),
      ),
