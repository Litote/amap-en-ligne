...
class OrganizationRepository {
  ...
  /// Updates the core identity fields of [currentOrg] and flushes the change.
  ///
  /// Trims [name] and [contactEmail]; converts empty-or-blank strings to null
  /// for the optional fields [timezone], [defaultLanguage], and [website].
  Future<void> updateIdentity({
    ...
    final updatedOrg = currentOrg.copyWith(
      ...
      timezone: _normalizeOptional(timezone),
      defaultLanguage: _normalizeOptional(defaultLanguage),
      website: _normalizeOptional(website),
    );
    return _submitOrgMutation(updatedOrg);
  }
  ...
  /// Assigns [memberId] as a coordinator by first fetching the fresh
  /// organization state from the stream.
  ///
  /// This variant prevents race conditions where the callback capturing the
  /// organization state executes after the widget has rebuilt with newer data.
  /// Safe to use from async contexts (e.g., bottom sheet selection callbacks).
  Future<void> assignCoordinatorById({
    required String organizationId,
    required String deliveryId,
    required String contractId,
    required String memberId,
  }) async {
    final freshOrg = await watch(organizationId).first;
  ...
  /// Unassigns [memberId] as a coordinator by first fetching the fresh
  /// organization state from the stream.
  ///
  /// This variant prevents race conditions where the callback capturing the
  /// organization state executes after the widget has rebuilt with newer data.
  /// Safe to use from async contexts (e.g., bottom sheet selection callbacks).
  Future<void> unassignCoordinatorById({
    required String organizationId,
    required String deliveryId,
    required String contractId,
    required String memberId,
  ...
  /// Updates the [DeliveryStatus] of a delivery, writes it optimistically,
  /// and enqueues an [Upsert] mutation.
  Future<void> updateDeliveryStatus({
    ...
    final updatedOrg = currentOrg.copyWith(
      deliveries: currentOrg.deliveries
          .map(
            (d) =>
                d.deliveryId == deliveryId ? d.copyWith(status: newStatus) : d,
          )
          .toList(),
    );
    return _submitOrgMutation(updatedOrg);
