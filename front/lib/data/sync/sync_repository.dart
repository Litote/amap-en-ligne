...
class SyncRepository {
  ...
  Future<SyncOutcome> _doSync({required String tenantId}) async {
    try {
      ...
      // Catches both Exception and Error subtypes (e.g. StateError from missing
      // entity handlers) that would otherwise escape uncaught.
      await Sentry.captureException(e, stackTrace: s);
  ...
  Future<void> _reconcileMutations(
    ...
    // INTERNAL_ERROR rejections are transient server-side failures: the
    // mutation stays queued and is resent on the next sync (offline-like
    // semantics). Every other outcome — APPLIED or definitive rejection —
    // drains its pending row.
    final acknowledged = outcomes
        .where((o) => o.error?.code != MutationErrorCode.internalError)
        .toList();
    if (acknowledged.isNotEmpty) {
      await _db.drainPendingMutations(acknowledged.map((o) => o.clientOpId));
    }
  }
