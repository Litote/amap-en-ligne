...
extension OrganizationDeliveryProductsX on Organization {
  List<OrgProduct> productsForDelivery(Delivery delivery) {
    final selectedProductTypeIds = delivery.basketDescriptions
        .map((description) => description.productTypeId)
        .toSet();
    final visibleProducts = selectedProductTypeIds.isEmpty
        ? products
        : products.where(
  ...
  List<String> productNamesForDelivery(Delivery delivery) =>
      productsForDelivery(delivery).map((product) => product.name).toList();
}
