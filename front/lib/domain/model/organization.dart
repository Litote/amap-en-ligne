...
extension OrganizationDeliveryProductsX on Organization {
  ...
  /// Product names attached to a single delivery-contract [dc]: the products
  /// referenced by the linked [Contract]'s product prices, or every product of
  /// its producer for a legacy price-less contract. Empty when the contract is
  /// unknown in [contracts] or resolves to no product.
  List<String> productNamesForDeliveryContract(
    DeliveryContract dc, {
    List<Contract> contracts = const [],
  }) {
    final contract = contracts
        .where((c) => c.contractId == dc.contractId)
        .firstOrNull;
    if (contract == null) return const [];

    final allowedProductTypeIds = contract.productPrices.isNotEmpty
    ...
    final seen = <String>{};
    for (final product in products) {
      if (!allowedProductTypeIds.contains(product.productTypeId)) continue;
      if (!seen.add(product.productTypeId)) continue;
