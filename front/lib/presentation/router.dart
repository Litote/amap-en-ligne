...
String? computeRouterRedirect(
  ...
  // OWNER guard: /owner/users* and /owner/invite-administrator require Role.owner.
  if (location.startsWith('/owner/users') ||
      location == '/owner/invite-administrator') {
    if (producerAccountId == null) {
      return '/login?from=${Uri.encodeQueryComponent(uri.toString())}';
    }
    if (role != UserRole.owner) return '/login';
  }

  if (producerAccountId != null && location == '/login') {
  ...
  if (producerAccountId == null &&
      !_publicRoutes.contains(location) &&
      location != '/') {
    if (logoutRequested) return '/login';
    return '/login?from=${Uri.encodeQueryComponent(uri.toString())}';
  }
  return null;
}
...
String _landingRouteFor({required UserRole? role, required bool isAdmin}) {
  return switch (role) {
    ...
    UserRole.producer => '/product-types',
    UserRole.admin ||
    UserRole.coordinator ||
    UserRole.volunteer ||
    UserRole.memberNoRole => '/dashboard',
    null => isAdmin ? '/admin/organization-requests' : '/product-types',
  };
}
