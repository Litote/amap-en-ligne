...
class _InstanceStats {
  ...
  factory _InstanceStats.from({
    ...
    for (final request in organizationRequests) {
      ...
      final submittedAt = DateTime.tryParse(request.submittedAt);
      if (submittedAt != null &&
    ...
    for (final request in producerRequests) {
      switch (request.status) {
        case ProducerRequestStatus.pendingValidation:
          pending += 1;
        case ProducerRequestStatus.approved:
        case ProducerRequestStatus.rejected:
          break;
      }
      final submittedAt = DateTime.tryParse(request.submittedAt);
      if (submittedAt != null &&
    ...
    return _InstanceStats(
      ...
      activeOrganizations: approved,
      requestsThisMonth: monthTotal,
      rejectedThisMonth: monthRejected,
    );
  }
