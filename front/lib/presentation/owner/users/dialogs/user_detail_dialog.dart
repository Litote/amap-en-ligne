  ...
  Widget build(BuildContext context) {
    return _SectionCard(
      ...
      children: [
        ...
        TextButton(
          key: const Key('view_producer_link'),
          onPressed: null,
          child: const Text('Voir la fiche producteur →'),
        ),
      ],
    );
  }
...
class _DangerZoneState extends State<_DangerZone> {
  ...
  Widget _suspendOrReactivateButton() {
    ...
    final tooltip = !_supportsLifecycle
        ? 'Action indisponible pour cet utilisateur'
        : _isSelf
  ...
  Future<void> _runSuspend() async {
    final dialog = _isProducer
        ? ConfirmSuspendProducerDialog(userRow: widget.row)
        : _isAmapMember
        ? ConfirmSuspendMemberDialog(userRow: widget.row)
        : ConfirmSuspendOwnerDialog(userRow: widget.row);
  ...
  Future<void> _runReactivate() async {
    final dialog = _isProducer
        ? ConfirmReactivateProducerDialog(userRow: widget.row)
        : _isAmapMember
        ? ConfirmReactivateMemberDialog(userRow: widget.row)
        : ConfirmReactivateOwnerDialog(userRow: widget.row);
    final confirmed = await showDialog<bool>(
      context: context,
      useRootNavigator: true,
      builder: (_) => dialog,
    ...
    await _executeMutation(
      action: _isAmapMember
          ? () => context.read<MemberRepository>().reactivate(
              memberId: widget.row.memberships.first.memberId,
              organizationId: widget.row.memberships.first.organizationId,
            )
          : _isProducer
          ? () => context.read<ProducerAccountRepository>().reactivate(
              widget.row.producerAccountId ?? widget.row.ownerId,
            )
          : () =>
  ...
  Future<void> _runDelete() async {
    final dialog = _isProducer
        ? ConfirmDeleteProducerDialog(userRow: widget.row)
        : _isAmapMember
        ? ConfirmDeleteMemberDialog(userRow: widget.row)
        : ConfirmDeleteOwnerDialog(userRow: widget.row);
    final confirmed = await showDialog<bool>(
      context: context,
      useRootNavigator: true,
      builder: (_) => dialog,
    ...
    await _executeMutation(
      action: _isAmapMember
          ? () => context.read<MemberRepository>().delete(
              memberId: widget.row.memberships.first.memberId,
              organizationId: widget.row.memberships.first.organizationId,
            )
          : _isProducer
          ? () => context.read<ProducerAccountRepository>().delete(
              widget.row.producerAccountId ?? widget.row.ownerId,
            )
          : () => context.read<OwnerRepository>().delete(widget.row.ownerId),
