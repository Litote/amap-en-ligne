...
class UserListBloc extends Bloc<UserListEvent, UserListState> {
  ...
  List<UserRow> _applyFilters(
    ...
    if (searchQuery.isNotEmpty) {
      ...
      result = result.where((r) => _normalize(r.firstName).contains(query) ||
            _normalize(r.lastName).contains(query) ||
            _normalize(r.email).contains(query)).toList();
    }

    if (amapId != null) {
      result = result.where((r) => r.memberships.any((m) => m.organizationId == amapId)).toList();
    }
