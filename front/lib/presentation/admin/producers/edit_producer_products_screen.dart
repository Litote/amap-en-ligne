...
class _EditProducerProductsViewState extends State<_EditProducerProductsView> {
  ...
  static bool _isLoadedState(ProducerManagementState state) =>
      state is ProducerManagementDetailLoaded ||
      state is ProducerManagementListLoaded;

  static bool _actionInProgressOf(
    ProducerManagementState state,
  ) => switch (state) {
    ProducerManagementDetailLoaded(:final actionInProgress) => actionInProgress,
    ProducerManagementListLoaded(:final actionInProgress) => actionInProgress,
    _ => false,
  };

  static String? _actionErrorOf(ProducerManagementState state) =>
      switch (state) {
  ...
  void _onStateChanged(BuildContext context, ProducerManagementState state) {
    if (!_saveRequested) return;
    if (_actionInProgressOf(state)) return;
    final actionError = _actionErrorOf(state);
    if (actionError != null) {
  ...
  @override
  Widget build(BuildContext context) => BlocConsumer<ProducerManagementBloc, ProducerManagementState>(
      listenWhen: (_, current) => _isLoadedState(current),
          ...
          children: [
            CheckboxListTile(
              ...
              subtitle: product.description != null
                  ...
                  : null,
              onChanged: (checked) => onSelectionChanged(product, checked: checked),
            ),
...
class _ManagedProductsBody extends StatelessWidget {
  ...
  Widget _addProductButton() => Padding(
  ...
  Widget _productCard(ProducerProduct product, int index) {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        trailing: _productActions(index),
      ),
    );
  }
