...
class _UserManagementView extends StatelessWidget {
  ...
  static bool _listenWhen(
    ...
    UserManagementState current,
  ) {
    if (previous is! UserManagementLoaded || current is! UserManagementLoaded) {
      return false;
    }
    if (!previous.showingInviteForm && current.showingInviteForm) return true;
    if (!previous.inviteSuccess && current.inviteSuccess) return true;
    if (previous.feedbackMessage != current.feedbackMessage &&
        current.feedbackMessage != null) {
      return true;
    }
    return previous.editingMember != current.editingMember;
  }

  void _onStateChanged(BuildContext context, UserManagementState state) {
  ...
  Widget _inviteFab(BuildContext context) => FloatingActionButton(
  ...
  @override
  Widget build(BuildContext context) => BlocListener<UserManagementBloc, UserManagementState>(
...
class _LoadedBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // an auto PENDING_ACTIVATION invitation). Suppress the duplicate member row
    // while the invitation is pending — the person reappears as a member tile
    // once they activate (and the invitation is hidden), matching the normal
    // invite lifecycle.
    final pendingInvitationEmails = state.memberInvitations
  ...
  List<Member> _filteredMembers(
    ...
    Set<String> pendingInvitationEmails,
  ) {
    ...
    // Drop members that are already represented by a still-pending invitation
    // tile (deduplication — see the call site).
    if (pendingInvitationEmails.isNotEmpty) {
      result = result.where((m) {
        final email = m.email?.toLowerCase();
        return email == null || !pendingInvitationEmails.contains(email);
      }).toList();
    }
...
class _MemberInvitationTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Build trailing actions if invitation is pending
    final trailingActions =
        invitation.status == InvitationStatus.pendingActivation
        ? Row(
...
class _EditRolesDialog extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => BlocBuilder<UserManagementBloc, UserManagementState>(
      builder: (context, state) {
...
class _InviteDialog extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => BlocConsumer<UserManagementBloc, UserManagementState>(
