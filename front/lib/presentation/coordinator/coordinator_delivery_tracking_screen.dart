      ...
      builder: (context, snapshot) {
        ...
        return StreamBuilder<List<Member>>(
          ...
          builder: (context, membersSnapshot) {
            ...
            final isLive = delivery.status == DeliveryStatus.inProgress;
            final statusLabel = switch (delivery.status) {
            ...
            return ConnectedScaffold(
              ...
              body: BlocListener<SyncBloc, SyncState>(
                listenWhen: (_, current) =>
                    ...
                    current is SyncSucceeded &&
                        current.rejectedMutations.isNotEmpty,
                listener: (context, state) {
                  ...
                  if (state is SyncSucceeded &&
                      state.rejectedMutations.isNotEmpty) {
                    messenger.showSnackBar(
                      const SnackBar(
                        content: Text(
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          _BasketPickupSection(org: org, delivery: delivery),
          const SizedBox(height: 16),
          _ProgressionSection(delivery: delivery),
        ],
      ),
    );
  }
...
class _BasketPickupSection extends StatelessWidget {
  const _BasketPickupSection({required this.org, required this.delivery});
...
class _ProgressionSection extends StatelessWidget {
  const _ProgressionSection({required this.delivery});

  final Delivery delivery;

  @override
  Widget build(BuildContext context) {
    final requiredVolunteers = deliveryVolunteerStaffing(delivery).required;
    var presentVolunteers = 0;
    for (final contract in delivery.contracts) {
      for (final slot in contract.slots) {
        if (slot.status == SlotStatus.cancelled) continue;
        for (final registration in slot.registrations) {
          if (_isPresentStatus(registration.status)) {
            presentVolunteers++;
          }
        }
      }
