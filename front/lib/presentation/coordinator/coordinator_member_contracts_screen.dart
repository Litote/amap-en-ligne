...
class _CoordinatorMemberContractsScreenState
    ...
    return ConnectedScaffold(
      ...
      body: StreamBuilder<Organization?>(
        ...
        builder: (context, organizationSnapshot) {
          ...
          return _buildMemberStream(context, organization);
        },
      ),
    );
  }

  Widget _buildMemberStream(BuildContext context, Organization organization) => StreamBuilder<List<Member>>(
      stream: context.read<MemberRepository>().watch(widget.tenantId),
      builder: (context, memberSnapshot) {
        if (!memberSnapshot.hasData) {
          return const Center(child: CircularProgressIndicator());
        }
        return _buildContractStream(
          context,
          organization,
  ...
  Widget _buildContractStream(
    ...
    List<Member> rawMembers,
      ...
      builder: (context, contractSnapshot) {
        ...
        final data = _buildMemberContractsData(
          organization,
          rawMembers,
        ...
        return ContractEndedListener(
          child: LayoutBuilder(
            builder: (context, constraints) =>
                _buildWorkspace(context, constraints, data),
          ),
        );
      },
    );

  _MemberContractsData _buildMemberContractsData(
    Organization organization,
    List<Member> rawMembers,
    List<Contract> contracts,
  ) {
    ...
    return (
      ...
      activeAssignments: _countAssignments(
        contracts,
        selectedMember,
        ContractStatusView.active,
      ),
      upcomingAssignments: _countAssignments(
        contracts,
        selectedMember,
  ...
  int _countAssignments(
    List<Contract> contracts,
    Member? selectedMember,
    ContractStatusView view,
  ) => contracts
        .where(
          (contract) =>
              ...
              contractStatusView(contract) == view,
        )
        .length;
    ...
    if (constraints.maxWidth >= _desktopBreakpoint) {
      return Row(
        children: [
          SizedBox(
            width: 340,
            child: _buildMemberListPanel(data, shrinkWrap: false),
          ),
          const VerticalDivider(width: 1),
          Expanded(child: _buildMemberDetail(context, data, shrinkWrap: false)),
        ],
      );
    }
  ...
  Widget _buildMemberListPanel(
    ...
    required bool shrinkWrap,
      ...
      selectedMemberId: _selectedMemberId ?? data.selectedMember?.memberId,
  ...
  Widget _buildMemberDetail(
    ...
    final selectedMember = data.selectedMember;
    if (selectedMember == null) return const _NoMemberSelected();
    return _MemberContractDetail(
      member: selectedMember,
      contracts: data.contracts,
      organization: data.organization,
      allMembers: data.members,
      onManageSharedBasket: (contract) => _manageSharedBasket(
        context,
        member: selectedMember,
        contract: contract,
        organization: data.organization,
      ...
      shrinkWrap: shrinkWrap,
      subscriptionKeysByContract: _subscriptionKeysByContract,
      editingAssignedContractId: _editingAssignedContractId,
      editingSubscriptionKeys: _editingSubscriptionKeys,
      onSelectionChanged: (contractId, {required selected}) =>
          _onContractSelectionChanged(
            contractId,
            selected,
            data.contracts,
            data.organization,
          ),
      onSubscriptionChanged: _onSubscriptionChanged,
      ...
      onEditSubscriptionChanged: _onEditSubscriptionChanged,
      onSaveEdit: () => _saveEditSubscription(
        context,
        member: selectedMember,
        contract: data.contracts.firstWhere(
          (c) => c.contractId == _editingAssignedContractId,
        ),
        organization: data.organization,
      ),
  ...
  void _onSubscriptionChanged(String contractId, Set<String> keys) {
    setState(() => _subscriptionKeysByContract[contractId] = keys);
  }

  void _onEditSubscriptionChanged(Set<String> keys) {
    setState(() => _editingSubscriptionKeys = keys);
  }
  ...
  void _onContractSelectionChanged(
    String contractId,
    bool selected,
    List<Contract> contracts,
    Organization organization,
    ...
    setState(() {
      if (selected) {
        ...
        _maybePreselectSoleSubscription(contractId, contracts, organization);
      } else {
        _selectedContractIds.remove(contractId);
        _subscriptionKeysByContract.remove(contractId);
      }
    });
  }

  void _maybePreselectSoleSubscription(
    String contractId,
    List<Contract> contracts,
    Organization organization,
  ) {
    final existing = _subscriptionKeysByContract[contractId];
    if (existing != null && existing.isNotEmpty) return;
    final contract = contracts.firstWhere((c) => c.contractId == contractId);
    final options = subscriptionOptionsFromPrices(
      contract.productPrices,
      organization,
    );
    if (options.length == 1) {
      _subscriptionKeysByContract[contractId] = {options.first.key};
    }
  }
            ...
            if (members.isEmpty)
              ...
              ListView.separated(
                ...
                itemBuilder: (context, index) {
                  ...
                  final count = contracts.where((contract) => contract.members.any(
                      (entry) => entry.memberId == member.memberId,
                    )).length;
                  return ListTile(
...
class _MemberContractDetail extends StatelessWidget {
  ...
  final bool shrinkWrap;
  final void Function(String contractId, {required bool selected}) onSelectionChanged;
  ...
  @override
  Widget build(BuildContext context) {
    final assignedContracts = _sortedByLabel(
      contracts.where(
        (c) => c.members.any((e) => e.memberId == member.memberId),
      ),
    );
    final availableContracts = _sortedByLabel(
      contracts.where(
        (c) =>
            !c.members.any((e) => e.memberId == member.memberId) &&
  ...
  List<Contract> _sortedByLabel(Iterable<Contract> source) => source.toList()..sort(
  ...
  Widget _buildAssignedContractCard(BuildContext context, Contract contract) {
    ...
    final isEditing = editingAssignedContractId == contract.contractId;
    return Padding(
      ...
      child: Card.outlined(
        child: Column(
          ...
          children: [
            ...
            if (isEditing)
              _buildEditSubscriptions(contract)
  ...
  Widget _buildEditSubscriptions(Contract contract) => Padding(
      padding: const EdgeInsets.only(left: 16, right: 16, bottom: 4),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          for (final option in subscriptionOptionsFromPrices(
            contract.productPrices,
            organization,
          ))
            CheckboxListTile(
              value: editingSubscriptionKeys.contains(option.key),
              controlAffinity: ListTileControlAffinity.leading,
              contentPadding: EdgeInsets.zero,
              onChanged: saving
                  ? null
                  : (value) =>
                        _toggleEditSubscription(option.key, value ?? false),
  ...
  void _toggleEditSubscription(String key, bool selected) {
    final updated = Set<String>.from(editingSubscriptionKeys);
    if (selected) {
      updated.add(key);
    } else {
      updated.remove(key);
    ...
    if (availableContracts.isEmpty) {
      return const [
        Padding(
          ...
          child: Center(
            child: Text('Aucun autre contrat disponible pour cet Amapien.'),
          ),
        ),
      ];
    }
  ...
  Widget _buildAvailableContractTile(BuildContext context, Contract contract) {
    ...
    final subscriptionKeys =
        subscriptionKeysByContract[contract.contractId] ?? <String>{};
    final options = subscriptionOptionsFromPrices(
      contract.productPrices,
      organization,
    );
      ...
      children: [
        CheckboxListTile(
          ...
          onChanged: saving
              ? null
              : (value) =>
                    onSelectionChanged(contract.contractId, selected: value ?? false),
          title: Text(contractProductLabel(contract, organization)),
              ...
              children: [
                for (final option in options)
                  CheckboxListTile(
                    ...
                    onChanged: saving
                        ? null
                        : (value) => _toggleAvailableSubscription(
                            contract.contractId,
                            subscriptionKeys,
                            option.key,
                            value ?? false,
  ...
  void _toggleAvailableSubscription(
    String contractId,
    Set<String> subscriptionKeys,
    String key,
    bool selected,
  ) {
    final updated = Set<String>.from(subscriptionKeys);
    if (selected) {
      updated.add(key);
    } else {
      updated.remove(key);
    }
    onSubscriptionChanged(contractId, updated);
  }
}
...
class SharedBasketDialogResult {
  const SharedBasketDialogResult({
        ...
        if (hasExisting)
          TextButton(
            onPressed: () => Navigator.of(
              context,
            ).pop(const SharedBasketDialogResult(memberIds: [], remove: true)),
          ...
          onPressed: _selected.isEmpty
              ...
              : () => Navigator.of(context).pop(
                  SharedBasketDialogResult(
                    memberIds: [widget.anchorMember.memberId, ..._selected],
                  ),
                ),
