...
class _TimeSlotFormScreenState extends State<TimeSlotFormScreen> {
  ...
  void _applyTemplateSelection(
    ...
    setState(() {
      ...
      if (!_isEditing && template != null && !_hasEditedScheduledTime) {
        _scheduledTime = _parseTemplateStartTime(template.standardStartTime);
      }
      if (template != null && !_hasEditedMinVolunteers) {
  ...
  /// Materialises the default volunteer slots on the first contract link
  /// when the delivery has none: volunteers cannot register without a slot
  /// (the back only lets privileged callers create them), so every privileged
  /// save backfills one.
  List<DeliveryContract> _withDefaultSlots(
    ...
    return [
      contracts.first.copyWith(
        slots: defaultVolunteerSlots(
          scheduled: scheduled,
          requiredVolunteers: minVolunteers,
          template: template,
          standardEndTimeOverride: standardEndTimeOverride,
          volunteerArrivalTimeOverride: volunteerArrivalTimeOverride,
          earlySlotOverride: earlySlotOverride,
        ),
      ),
      ...contracts.skip(1),
    ];
  ...
  Future<void> _submit(
    ...
    if (_hasSameDayConflict(org, _scheduledDate!)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Une livraison existe déjà ce jour-là pour cette AMAP.',
          ),
        ),
      );
      return;
    ...
    // first.
    if (_isEditing) {
      final existing = existingDelivery;
      if (existing != null) {
        final scheduleDelta = scheduled.difference(
          DateTime.parse(existing.scheduledDate),
        );
        final timesChanged =
            ...
            _hasEditedEnd ||
            _hasEditedEarlySlot;
        final activeCount = _activeRegistrationCount(existing);
        if (timesChanged && activeCount > 0) {
      ...
      if (_isEditing) {
        ...
        final updated = existing.copyWith(
          ...
          contracts: _withRewrittenSlots(
            ...
            standardEndTimeOverride: endOverride,
            volunteerArrivalTimeOverride: arrivalOverride,
            earlySlotOverride: earlyOverride,
          ),
        );
  ...
  Widget _buildForm({
    ...
    required bool selectedTemplateExists,
    ...
    final visibleProducts = _visibleProductsForContracts(
      org,
      activeContracts,
      selectedContractIds,
    );
                        ...
                        return StreamBuilder<List<ProducerAccount>>(
                          ...
                          builder: (context, producersSnapshot) {
                            ...
                            return _buildForm(
                              ...
                              existingDelivery: existingDelivery,
                              templates: templates,
                              selectedTemplateExists: selectedTemplateExists,
                              allMembers: allMembers,
                              me: me,
                              contracts: contracts,
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            ...
            // (spec: between Contrats présents and Instructions spéciales)
            // ---------------------------------------------------------------
            _CoordinatorBlock(
              delivery: existingDelivery,
              previewContractNames: previewContractNames,
            ...
            if (existingDelivery != null)
              _SlotsBlock(delivery: existingDelivery!, org: org),
            const SizedBox(height: 16),
            TextFormField(
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        SwitchListTile(
          contentPadding: EdgeInsets.zero,
          title: const Text('Créneau matinal'),
        ...
        if (earlySlotEnabled) ...[
          ...
          TextFormField(
            ...
            decoration: const InputDecoration(
              labelText: 'Bénévoles max (créneau matinal)',
              border: OutlineInputBorder(),
            ),
            keyboardType: TextInputType.number,
            inputFormatters: [FilteringTextInputFormatter.digitsOnly],
          ...
          TextFormField(
            ...
            decoration: const InputDecoration(
              labelText: 'Explication (créneau matinal, facultatif)',
              border: OutlineInputBorder(),
            ),
            maxLines: 2,
          ),
...
class _ContractCoordinatorRow extends StatelessWidget {
  ...
  bool get _canSelfAssign {
    if (!_isActive) return false;
    if (me == null) return false;
    return !contract.coordinators.contains(me!.memberId);
  }
  ...
  void _showAdminPicker(BuildContext context, Map<String, Member> membersById) {
    // List all AMAP members with COORDINATOR role.
    final coordinators = allMembers
...
class _SlotsBlock extends StatelessWidget {
  const _SlotsBlock({required this.delivery, required this.org});
...
class _SlotRow extends StatelessWidget {
  ...
  int get _activeRegistrations => slot.registrations
      .where((r) => r.status != RegistrationStatus.cancelled)
      .length;
