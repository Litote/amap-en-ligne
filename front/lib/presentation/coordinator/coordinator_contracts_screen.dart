...
import 'package:amap_en_ligne/presentation/coordinator/contract_subscription_checkboxes.dart';
...
class _CoordinatorContractsScreenState
  ...
  Widget _buildContractStream(BuildContext context, Organization organization) => StreamBuilder<List<Contract>>(
  ...
  Widget _buildProducerStream(
    ...
    List<Contract> contracts,
    List<Member> members,
  ) => StreamBuilder<List<ProducerAccount>>(
  ...
  Widget _buildTemplateStream(
    ...
    data,
  ) => StreamBuilder<List<DeliveryTemplate>>(
  ...
  void _onProductIncludedToggled(String productTypeId, {required bool selected}) {
    setState(() {
  ...
  Widget _buildWorkspace(
    ...
    final form = _ContractEditor(
      ...
      onMemberToggled: (member, {required selected}) =>
...
class _ContractEditor extends StatelessWidget {
  ...
  final GlobalKey<CoordinatorSelectorState> coordinatorSelectorKey;
  ...
  final void Function(Member member, {required bool selected}) onMemberToggled;
  final ValueChanged<List<Member>> onSelectAllMembers;
  final ValueChanged<String> onMemberSearchChanged;
  final void Function(String memberId, String key, {required bool selected})
  ...
  Widget _buildStatusDropdown() => DropdownButtonFormField<ContractStatus>(
  ...
  Widget _buildMainContractSwitch() => SwitchListTile(
  ...
  List<Widget> _buildMemberTile(Member member, bool contractEnded) => [
      CheckboxListTile(
        ...
        onChanged:
            ...
            ? null
            : (value) => onMemberToggled(member, selected: value ?? false),
      ),
      if (selectedMemberIds.contains(member.memberId) &&
          selectedProducerAccountId != null)
        Padding(
          padding: const EdgeInsets.only(left: 32),
          child: ContractSubscriptionCheckboxes(
            ...
            organization: organization,
            saving: saving,
            onSubscriptionToggled: (key, {required selected}) =>
                onMemberSubscriptionChanged(member.memberId, key, selected: selected),
          ),
        ),
    ];

  List<ProductPrice> _currentProductPrices() => priceControllers.keys
        .map((key) {
          final parts = key.split(':');
          final productTypeId = parts[0];
          final basketSizeName = parts.length > 1 ? parts[1] : '';
