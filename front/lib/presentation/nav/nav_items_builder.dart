...
List<NavItem> buildNavItems(Set<Role> roles, VoidCallback onLogout) {
  ...
  result
    ..add(const NavItem.separator())
    ..addAll(_commonItems(onLogout));
  return result;
}
...
List<NavItem> buildNavItemsForRole(UserRole role, VoidCallback onLogout) => switch (role) {
...
const _kBasketExchangeRoute = '/basket-exchange';
const _kBasketExchangeLabel = 'Échange de paniers';

const _volunteerRoleItems = [
  NavItem(label: 'Accueil', icon: Icons.home, route: _kDashboardRoute),
  ...
  NavItem(
    label: _kBasketExchangeLabel,
    icon: Icons.swap_horiz,
    route: _kBasketExchangeRoute,
  ),
];
...
const _adminRoleItems = [
  ...
  NavItem(
    label: "Configuration de l'organisation",
    icon: Icons.settings,
