...
class AuthBloc extends Bloc<AuthEvent, AuthViewState> {
  ...
  void _onMemberNameUpdated(
    AuthMemberNameUpdated event,
    Emitter<AuthViewState> emit,
  ) {
    emit(state.copyWith(firstName: event.firstName, lastName: event.lastName));
  }

  void _onMemberRolesUpdated(
    AuthMemberRolesUpdated event,
    Emitter<AuthViewState> emit,
  ) {
    emit(state.copyWith(memberRoles: event.roles));
  }
