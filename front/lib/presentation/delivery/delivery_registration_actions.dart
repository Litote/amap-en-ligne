...
///
/// Prefers a contract whose [kind] slot still has free capacity (by the
/// denormalised [MemberSlot.currentRegistrations] field, matching the capacity
/// math used by the cards); falls back to the first contract.
String? _findContractIdForKind(Delivery delivery, SlotKind kind) {
  for (final contract in delivery.contracts) {
    for (final slot in contract.slots) {
      if (slot.slotKind == kind &&
          slot.currentRegistrations < slot.requiredVolunteers) {
        return contract.contractId;
      }
    }
  }
  return delivery.contracts.firstOrNull?.contractId;
}
...
void registerToSlotAction(
  ...
  final contractId = _findContractIdForKind(delivery, slotKind);
