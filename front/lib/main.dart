...
Future<void> main() async {
  ...
  try {
    ...
    // Push disabled — the app boots and works without it. Sentry is not
    // initialised yet at this point, so a debug log is the only trace.
...
class _AmapEnLigneAppState extends State<AmapEnLigneApp> {
  ...
  ServerConfig _resolveConfig() {
    if (widget.authServiceOverride != null) {
      final options = widget.serverCatalog();
...
class _AuthenticatedAppShellState extends State<_AuthenticatedAppShell> {
  ...
  late final _wiredAuthBloc = AuthBloc(
    ...
    db: widget.db,
    memberRepository: widget.memberRepo,
  ...
  Widget build(BuildContext context) => MultiRepositoryProvider(
    providers: [
      ...
      RepositoryProvider<ContractRepository>.value(value: widget.contractRepo),
      ...
      RepositoryProvider<DeviceTokenRepository>.value(value: _deviceTokenRepo),
