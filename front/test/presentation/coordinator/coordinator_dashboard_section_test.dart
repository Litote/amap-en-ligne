...
void main() {
  ...
  group('CoordinatorDashboardSection — ME PORTER COORDINATEUR', () {
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        when(() => memberRepo.watchMyMember(any())).thenAnswer(
          (_) => Stream.value(const Member(memberId: myId, organizationId: 'org-1')),
        );
        when(() => authService.currentState).thenReturn(
          AuthState.authenticated(
            producerId: 'tenant-1',
  ...
  group('CoordinatorDashboardSection — Suivre button', () {
    ...
    testWidgets('card shows Suivre button and navigates to tracking', (
      tester,
    ) async {
      const dId = 'd-track';
      final delivery = buildDelivery(
        ...
        scheduledDate: tomorrowIso(),
        contracts: [
          buildContract(contractId: 'c-1', coordinators: ['x']),
        ],
      );
      when(
        () => orgRepo.watch(any()),
        ...
        routes: [
          GoRoute(
            ...
            builder: (context, state) => const Scaffold(
              body: SingleChildScrollView(
                child: CoordinatorDashboardSection(tenantId: 'org-1'),
              ),
            ),
          ),
          GoRoute(
      ...
      expect(find.text('tracking-$dId'), findsOneWidget);
    });
  });
}
