...
const _pending = AdminMemberJoinRequest(
  requestId: 'req-1',
...
const _rejectedMutation = MutationOutcome(
  clientOpId: 'op-1',
...
void main() {
  ...
  group('approveRequested', () {
    blocTest<MembershipRequestsBloc, MembershipRequestsState>(
      ...
      seed: () => const MembershipRequestsState.loaded(requests: [_pending]),
      act: (bloc) =>
          bloc.add(const MembershipRequestsEvent.approveRequested(request: _pending)),
      expect: () => [
        isA<MembershipRequestsLoaded>().having(
    ...
    blocTest<MembershipRequestsBloc, MembershipRequestsState>(
      ...
      setUp: () {
        ...
        when(() => syncRepository.sync(tenantId: 'org-1')).thenAnswer(
          (_) async =>
              const SyncOutcome.success(rejectedMutations: [_rejectedMutation]),
        );
      },
      build: () => MembershipRequestsBloc(
        organizationId: 'org-1',
  ...
  group('rejectRequested', () {
    blocTest<MembershipRequestsBloc, MembershipRequestsState>(
      ...
      seed: () => const MembershipRequestsState.loaded(requests: [_pending]),
      act: (bloc) => bloc.add(
        const MembershipRequestsEvent.rejectRequested(
          request: _pending,
    ...
    blocTest<MembershipRequestsBloc, MembershipRequestsState>(
      ...
      seed: () => const MembershipRequestsState.loaded(requests: [_pending]),
      act: (bloc) =>
          bloc.add(const MembershipRequestsEvent.rejectRequested(request: _pending)),
      expect: () => [
        isA<MembershipRequestsLoaded>().having(
          (s) => s.actionInProgress,
          'actionInProgress',
