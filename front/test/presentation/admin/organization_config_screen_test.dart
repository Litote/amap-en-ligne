...
void main() {
  ...
  group('OrganizationConfigScreen', () {
    testWidgets('shows loading spinner while org is not yet available', (
      ...
      when(() => repo.watch(_tenantId)).thenAnswer((_) => const Stream.empty());
    ...
    testWidgets('tapping save calls updateIdentity with form values', (
      tester,
    ) async {
      when(() => repo.watch(_tenantId)).thenAnswer((_) => Stream.value(_org));
      when(
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await tester.enterText(find.byKey(const Key('org_config_name')), '   ');
    ...
    testWidgets('validates email format — shows error when email is invalid', (
      tester,
    ) async {
      when(() => repo.watch(_tenantId)).thenAnswer((_) => Stream.value(_org));
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await tester.enterText(find.byKey(const Key('org_config_email')), '');
