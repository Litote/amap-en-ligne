...
const _pending = AdminProducerRequest(
  requestId: 'req-1',
...
void main() {
  ...
  blocTest<ProducerRequestsBloc, ProducerRequestsState>(
    ...
    build: () => ProducerRequestsBloc(producerRequestRepository: repo),
    seed: () => const ProducerRequestsState.loaded(requests: [_pending]),
    act: (bloc) =>
        bloc.add(const ProducerRequestsEvent.approveRequested(request: _pending)),
    expect: () => [
      isA<ProducerRequestsLoaded>().having(
  ...
  blocTest<ProducerRequestsBloc, ProducerRequestsState>(
    ...
    seed: () => const ProducerRequestsState.loaded(requests: [_pending]),
    act: (bloc) =>
        bloc.add(const ProducerRequestsEvent.resendRequested(request: _pending)),
    expect: () => [
      isA<ProducerRequestsLoaded>().having(
        (s) => s.actionInProgress,
        'actionInProgress',
