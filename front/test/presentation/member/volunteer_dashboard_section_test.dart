...
void main() {
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        final contract = buildContract(
          contractId: 'c-1',
          // Use a different coordinator so that _kMemberId counts as a volunteer.
          coordinators: const ['coordinator-99'],
          slots: [slot1, slot2],
        );
        final delivery = buildDelivery(
          deliveryId: 'd-1',
  ...
  group('Coordinators section', () {
    testWidgets('shows the full coordinator name under its contract header', (
      tester,
    ) async {
      ...
      // The detailed section lists the contract name (header) and the full
      // coordinator name + phone, like the planning + tracking screens.
    ...
    testWidgets(
      'shows "Coordinateur à confirmer" when contract coordinators list is empty',
      (tester) async {
        // Slot needed so upcomingActiveDeliveries includes this delivery.
        final slot = buildSlot(requiredVolunteers: 3, currentRegistrations: 0);
        final contract = buildContract(
        ...
        // When no coordinator is assigned, the section shows the placeholder.
        expect(find.text('Légumes'), findsOneWidget);
        expect(find.textContaining('Coordinateur à confirmer'), findsOneWidget);
      },
    );

    testWidgets('lists each contract of the delivery with its coordinator', (
      tester,
    ) async {
      ...
      // Each contract appears with its own coordinator row.
      expect(find.text('Légumes'), findsOneWidget);
      expect(find.text('Pain'), findsOneWidget);
      expect(find.textContaining('Jean Morel'), findsOneWidget);
      expect(find.textContaining('Claire Petit'), findsOneWidget);
    });

    testWidgets('exposes the coordinator phone as a tel: link', (tester) async {
      final coordinator = _buildCoordinatorMember(
        memberId: 'coord-1',
        firstName: 'Jean',
        lastName: 'Morel',
  ...
  group('Suivre button (coordinator-only)', () {
    Delivery buildUpcomingDelivery() {
      final slot = buildSlot(requiredVolunteers: 3, currentRegistrations: 0);
      final contract = buildContract(
        contractId: 'c-1',
    ...
    testWidgets('volunteer-only member sees no Suivre button', (tester) async {
      await _pump(
        tester,
        orgRepo: orgRepo,
        memberRepo: memberRepo,
      ...
      orgStream.add(buildOrg(deliveries: [buildUpcomingDelivery()]));
      memberStream.add(_buildMember());
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 50));
    ...
    testWidgets('coordinator member sees Suivre button on cards', (
      tester,
    ) async {
      await _pump(
        tester,
      ...
      expect(find.text('Suivre'), findsAtLeastNWidgets(1));
    });
  });
}
