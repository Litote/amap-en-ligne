...
/// Current-month helper — used with [initialMonth] to pin the planning screen
/// to the current month and suppress auto-advance logic in tests that need to
/// exercise past deliveries without navigating.
...
void main() {
  ...
  group('MemberDeliveryPlanScreen', () {
    ...
    testWidgets(
      'volunteer-only member sees neither Modifier nor Suivre on delivery cards',
      (tester) async {
    ...
    testWidgets('coordinator sees Suivre button and can navigate to tracking', (
      tester,
    ) async {
      final delivery = buildDelivery(
        deliveryId: 'd-1',
      ...
      expect(find.text('tracking-d-1'), findsOneWidget);
    });

    testWidgets('admin without coordinator role sees no Suivre button', (
      tester,
    ) async {
      final delivery = buildDelivery(
        deliveryId: 'd-1',
    ...
    testWidgets(
      'registration targets the main contract slot, not a secondary contract',
      (tester) async {
        final mainSlot = buildSlot(requiredVolunteers: 2);
        final secondarySlot = buildSlot(requiredVolunteers: 2);
        // The secondary (Pain) link is listed FIRST so the legacy "first free
        // slot" logic would have mis-targeted it.
        final delivery = buildDelivery(
          deliveryId: 'd-1',
          scheduledDate: tomorrowIso(),
          contracts: [
        ...
        Contract season(String id, String name, {required bool main}) =>
            Contract(
              contractId: id,
              name: name,
              organizationId: 'org-1',
        ...
        when(() => contractRepo.watch(any())).thenAnswer(
          (_) => Stream.value([
            season('c-veg', 'Légumes', main: true),
            season('c-pain', 'Pain', main: false),
          ]),
        );
        ...
        await tester.tap(find.textContaining("S'INSCRIRE"));
        await tester.pump();
    ...
    testWidgets('footer shows ACCUEIL, MON HISTORIQUE, and enabled AIDE', (
      tester,
    ) async {
      await _pump(
        tester,
      ...
      final aideButton = tester.widget<OutlinedButton>(
        find.ancestor(
          of: find.text('AIDE'),
          matching: find.byType(OutlinedButton),
        ),
      );
      expect(aideButton.onPressed, isNotNull);
    });
  });
  ...
  group('Coordinators section', () {
    ...
    testWidgets(
      'resolves the contract name from the live catalog when the link '
      'description snapshot is blank',
      (tester) async {
        final coordinator = _buildCoordinator(
          memberId: 'coord-1',
          firstName: 'Jean',
        ...
        when(
          () => contractRepo.watch(any()),
        ).thenAnswer((_) => Stream.value([liveContract]));
        ...
        expect(find.textContaining('Panier hebdomadaire'), findsOneWidget);
      },
    );
  ...
  group('auto-advance to next month', () {
    String monthLabel(DateTime month) {
      final raw = DateFormat('MMMM yyyy', 'fr').format(month);
      return raw[0].toUpperCase() + raw.substring(1);
    }

    String isoAt(DateTime dt) =>
    ...
    testWidgets(
      'shows next month when last delivery of current month has passed',
      (tester) async {
        await _pump(
          tester,
          orgRepo: orgRepo,
          ...
          authService: authService,
          syncBloc: syncBloc,
          // No initialMonth — let the auto-advance logic decide.
        );
        await tester.pump();

        final now = DateTime.now();
        // Delivery at the start of the current month — always in the past.
        final pastInMonth = isoAt(DateTime(now.year, now.month, 1, 0, 0));
        orgStream.add(
          buildOrg(deliveries: [buildDelivery(scheduledDate: pastInMonth)]),
        );
        memberStream.add(_buildMember());
        await tester.pump();
        await tester.pump(const Duration(milliseconds: 50));
    ...
    testWidgets(
      'keeps current month when a future delivery exists this month',
      (tester) async {
        await _pump(
          tester,
          orgRepo: orgRepo,
        ...
        final now = DateTime.now();
        // Delivery at the last minute of the current month — always future.
        final lastOfMonth = DateTime(
          now.year,
          now.month + 1,
          0,
        ); // day 0 = last day
        final futureInMonth = isoAt(
          DateTime(
            lastOfMonth.year,
            lastOfMonth.month,
            lastOfMonth.day,
        ...
        orgStream.add(
          buildOrg(deliveries: [buildDelivery(scheduledDate: futureInMonth)]),
        );
        memberStream.add(_buildMember());
        await tester.pump();
        await tester.pump(const Duration(milliseconds: 50));
