...
void main() {
  ...
  blocTest<ItemTypesBloc, ItemTypesState>(
    'GIVEN a failing repository WHEN ItemTypeAdded THEN emits the generic error message',
    setUp: () => when(
      () => repo.updateItemTypes(any(), any()),
    ...
    expect: () => [
      isA<ItemTypesSaving>(),
      const ItemTypesState.error(message: kUnexpectedErrorMessage),
    ],
  );

  blocTest<ItemTypesBloc, ItemTypesState>(
    'GIVEN a failing repository WHEN ItemTypeRemoved THEN emits the generic error message',
    setUp: () => when(
      () => repo.updateItemTypes(any(), any()),
    ).thenThrow(Exception('boom')),
    build: buildBloc,
  ...
  blocTest<ItemTypesBloc, ItemTypesState>(
    'GIVEN a failing repository WHEN ItemTypeUpdated THEN emits the generic error message',
    setUp: () => when(
      () => repo.updateItemTypes(any(), any()),
    ).thenThrow(Exception('boom')),
    build: buildBloc,
