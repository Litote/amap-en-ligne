...
void main() {
  ...
  group('buildNavItemsForRole — common items', () {
    ...
    test('Préférences, Aide and Se déconnecter present for every role', () {
      for (final role in UserRole.values) {
        final labels = actionLabels(buildNavItemsForRole(role, noop));
        expect(labels, contains('Préférences'), reason: 'role=$role');
        expect(labels, contains('Aide'), reason: 'role=$role');
    ...
    test('Aide routes to /help for every role', () {
      for (final role in UserRole.values) {
        final items = buildNavItemsForRole(role, noop);
        final aide = items.firstWhere((i) => i.label == 'Aide');
        expect(aide.route, '/help', reason: 'role=$role');
      }
    });
