...
void main() {
  ...
  blocTest<DeliveryDescriptionBloc, DeliveryDescriptionState>(
    ...
    build: buildBloc,
    act: (bloc) => bloc.add(
      const DeliveryDescriptionEvent.requested(org: _org, deliveryId: 'd-1'),
    ),
    expect: () => [
  ...
  blocTest<DeliveryDescriptionBloc, DeliveryDescriptionState>(
    'GIVEN an unknown delivery WHEN Requested THEN emits the generic error message',
    build: buildBloc,
    act: (bloc) => bloc.add(
      const DeliveryDescriptionEvent.requested(org: _org, deliveryId: 'unknown'),
    ),
    expect: () => [
      const DeliveryDescriptionState.error(message: kUnexpectedErrorMessage),
    ],
  );

  blocTest<DeliveryDescriptionBloc, DeliveryDescriptionState>(
    'GIVEN a failing repository WHEN SaveRequested THEN emits the generic error message',
    setUp: () => when(
      () => orgRepo.updateDeliveryDescription(
        currentOrg: any(named: 'currentOrg'),
        deliveryId: any(named: 'deliveryId'),
