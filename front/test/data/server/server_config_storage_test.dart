...
void main() {
  ...
  test('write then read round-trips termsUrl and guideUrl', () async {
    const config = GoTrueServerConfig(
      id: 'preset-a',
      name: 'Preset A',
      backendUrl: 'http://a',
      gotrueUrl: 'http://a-auth',
      termsUrl: 'http://a/cgu.html',
      guideUrl: 'http://a/guide/',
    );
    final storage = await _newStorage();
    await storage.write(config);
    final restored = storage.read();
    expect(restored?.termsUrl, 'http://a/cgu.html');
    expect(restored?.guideUrl, 'http://a/guide/');
  });
}
