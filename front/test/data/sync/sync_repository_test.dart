...
void main() {
  ...
  group('mutation reconciliation', () {
    ...
    test(
      'REJECTED with INTERNAL_ERROR keeps the pending mutation queued for retry',
      () async {
        final tmp = buildProductType(productTypeId: 'tmp_retry');
        await db.upsertProductType(tmp);
        final failing = buildProductTypeUpsertMutation(productType: tmp);
        await db.enqueuePendingMutation(
          failing,
          scopeKey: testProducerScopeKey,
        );
        final drained = buildProductTypeDeleteMutation(
          clientOpId: 'op-drained',
          entityId: 'pt-old',
        );
        await db.enqueuePendingMutation(
          drained,
        ...
        // The internally-failed mutation stays queued (it will be resent on
        // the next sync); the applied one is drained.
        final pending = await db.readPendingMutations();
        expect(pending, hasLength(1));
