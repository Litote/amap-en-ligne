...
void main() {
  ...
  test(
    'updateDeliveryStatus updates the delivery status and enqueues Upsert',
    () async {
      const deliveryId = 'd-1';
      ...
      final stored = await db.watchOrganization(_testOrgId).first;
      expect(stored, isNotNull);
      expect(stored!.deliveries.single.status, DeliveryStatus.completed);
      ...
      expect(upsert.payload, isA<OrganizationPayload>());
      final org = (upsert.payload as OrganizationPayload).organization;
      expect(org.deliveries.single.status, DeliveryStatus.completed);
    },
  );
