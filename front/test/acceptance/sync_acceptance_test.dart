...
void main() {
  ...
  final internalErrorStory = _loadStory(
  ...
  test('${internalErrorStory.title} [${internalErrorStory.id}]', () async {
    ...
    api = _ScriptedSyncApi([
      _ExpectedSyncCall.response(
        label: '${internalErrorStory.id} bootstrap',
        request: const SyncRequest(),
        response: const SyncResponse(
          authorizedScopes: [producerScope],
          results: {
    ...
    api
      ..enqueue(
        _ExpectedSyncCall.response(
          label: '${internalErrorStory.id} internal error',
          request: SyncRequest(
            cursors: const {producerScope: 'c0'},
            mutations: pending,
          ),
          response: const SyncResponse(
            ...
            mutations: [
              MutationOutcome(
                ...
                error: MutationError(
                  code: MutationErrorCode.internalError,
      ...
      ..enqueue(
        _ExpectedSyncCall.response(
          label: '${internalErrorStory.id} retry applied',
          request: SyncRequest(
            cursors: const {producerScope: 'c0'},
            mutations: pending,
          ),
    ...
    // First sync: the server fails internally — the optimistic row survives
    // and the mutation stays queued for retry.
    final firstOutcome = await syncRepo.sync(tenantId: tenant);
    expect(firstOutcome, isA<SyncSuccess>());
    expect(
      (await db.watchProductTypes(tenant).first).single.productTypeId,
      optimistic.productTypeId,
    );
    ...
    // Second sync resends the identical mutation (same client_op_id) and the
    // server applies it: the tmp_ id is remapped and the queue drains.
    final retryOutcome = await syncRepo.sync(tenantId: tenant);

    expect(retryOutcome, isA<SyncSuccess>());
    final rows = await db.watchProductTypes(tenant).first;
    expect(rows.single.productTypeId, 'pt-1');
    expect(await db.readPendingMutations(), isEmpty);
  });
