...
void main() {
  ...
  test('sync detects member role updates and signals refresh needed', () async {
    syncRepo = SyncRepository(db: db, api: api = _ScriptedSyncApi([]));

    // Pre-populate drift with initial state (admin only)
    final initialMember = member(roles: {Role.admin});
    await db.upsertMember(tenant, initialMember);
    await db.writeCursor('organization:org-1', 'c1');

    // Server returns the member with added coordinator role
    final updatedMember = member(roles: {Role.admin, Role.coordinator});
    api.enqueue(
      _ExpectedSyncCall.response(
        label: 'sync-role-update',
        request: const SyncRequest(cursors: {'organization:org-1': 'c1'}),
        response: SyncResponse(
          authorizedScopes: const ['organization:org-1'],
          results: {
            'organization:org-1': IncrementalScopeSyncResult(
              changes: [
                Change(
                  op: ChangeOp.upsert,
                  entityType: EntityType.member,
                  entityId: memberId,
                  payload: MemberPayload(member: updatedMember),
                  producedAt: DateTime.now().millisecondsSinceEpoch,
                ),
              ],
              nextCursor: 'c2',
            ),
    ...
    expect(outcome, isA<SyncSuccess>());
    // The key assertion: memberOrOwnerUpdated should be true
    expect((outcome as SyncSuccess).memberOrOwnerUpdated, isTrue);
    // And the member should be updated in the database
    final memberInDb = await db.getMember(tenant, memberId);
    expect(memberInDb?.roles, containsAll([Role.admin, Role.coordinator]));
  });
}
