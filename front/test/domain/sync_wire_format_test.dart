...
void main() {
  ...
  group('EntityPayload (sealed)', () {
    ...
    test('Contract legacy JSON without status defaults to IN_PREPARATION', () {
      ...
      // Legacy JSON without is_main_contract defaults to false.
      expect(contract.isMainContract, isFalse);
    });

    test('Contract with is_main_contract round-trip', () {
      const json = {
        'type': 'Contract',
        'contract': {
          'contract_id': 'c-veg',
      ...
      final payload = EntityPayload.fromJson(json);
      final contract = (payload as ContractPayload).contract;
      expect(contract.isMainContract, isTrue);
      final wire = wireOf(payload);
      final inner = wire['contract'] as Map<String, dynamic>;
      expect(inner['is_main_contract'], true);
    });
    ...
    group('OrganizationRequest payload', () {
      ...
      group('ProducerRequest payload', () {
        ...
        test('mutation write matches expected wire shape', () {
          ...
          const payload = ProducerRequestPayload(producerRequest: request);
          final wire = wireOf(payload);
    ...
    group('ErrorReport / ErrorReportPayload', () {
      test('round-trip with all fields', () {
        ...
        const payload = ErrorReportPayload(errorReport: report);
        final wire = wireOf(payload);
  ...
  group('MutationOutcome', () {
    ...
    test(
      'REJECTED with INTERNAL_ERROR error code round-trips the exact wire string',
      () {
        const json = {
          'client_op_id': 'op-6',
          'status': 'REJECTED',
        ...
        final outcome = MutationOutcome.fromJson(json);
        expect(outcome.status, MutationStatus.rejected);
        expect(outcome.error?.code, MutationErrorCode.internalError);
        final wire = wireOf(outcome);
        expect(
          (wire['error'] as Map<String, dynamic>)['code'],
          'INTERNAL_ERROR',
  ...
  group('SyncRequest', () {
    ...
    test('encodes a null cursor (bootstrap signal)', () {
      const request = SyncRequest(cursors: {'organization:org-1': null});
    ...
    test('encodes a tmp_ Upsert mutation in full wire form', () {
      ...
      const request = SyncRequest(
        mutations: [
          ClientMutation(
            clientOpId: 'op-1',
            op: Upsert(payload: ProductTypePayload(productType: tmp)),
          ),
        ],
      );
