...
│  │                                                         │ │
│  │  Produits présents (groupés par contrat coché) :        │ │
│  │     🥕 Légumes de saison                                │ │
│  │        ✅ Tomates                                        │ │
│  │     🥚 Œufs fermiers                                     │ │
│  │        ✅ Œufs                                           │ │
...
- **Bénévoles minimum** : Nombre requis pour assurer la livraison ; prérempli depuis le nombre de bénévoles souhaité du template sélectionné. Ce champ n'est affiché que si la livraison est liée à au moins un contrat principal (*CONTRACT* `is_main_contract`) — voir « Matérialisation des créneaux bénévoles »
...
Seuls les **contrats principaux** (*CONTRACT* avec `is_main_contract`) mobilisent des bénévoles. Le champ **Bénévoles minimum** et le bloc **Horaires des créneaux** ne sont donc affichés que lorsque la livraison est liée à au moins un contrat principal ; sinon aucun créneau bénévole n'est proposé ni créé (les contrats secondaires ne mobilisent que le coordinateur).

Les amapiens ne peuvent s'inscrire que sur un créneau (*MEMBER_SLOT*) existant — le serveur interdit la création de créneau à un appelant `VOLUNTEER`. Chaque enregistrement du formulaire (création ou modification) par un coordinateur/admin matérialise donc les créneaux par défaut **lorsque la livraison n'en possède encore aucun**, sur sa livraison-contrat (*DELIVERY_CONTRACT*) principale (à défaut, lorsqu'aucun contrat lié n'est principal, aucun créneau n'est créé) :
